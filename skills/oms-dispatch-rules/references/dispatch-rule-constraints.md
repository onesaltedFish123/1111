...
这个文档汇总了当前仓库里已经写死的分仓规则约束，生成内部执行草案前要先过一遍。  
默认不要把这些实现细节逐条转述给客户。

来源主要是：

- `OrderDispatchPageDOServiceImpl`
- `OrderDispatchCustomRuleServiceImpl`（历史 `custom rule` 参考）
...
## 4. Legacy: Custom Rule 的现实限制（仅排查历史配置时参考）
...
- 明确标出缺失项
- 明确标出“需要真实线上数据校验”的部分
- 如果需求属于 `custom rule / 条件路由`，优先说明“该能力已弃用，不再展示”
